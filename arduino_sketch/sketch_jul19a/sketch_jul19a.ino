#define FW_VERSION "FW 19.07.2019"
...
//-----------------------------------------------------------------------------
boolean fan_cmd() {
  boolean cmd = ((ctrl==0) && local_fan) || ((ctrl==1) && rem_fan) || ((ctrl==2) && man_fan);
  return cmd;
}
...
//-----------------------------------------------------------------------------
boolean valve_cmd() {
  boolean cmd = ((ctrl==0) && local_valve) || ((ctrl==1) && rem_valve) || ((ctrl==2) && man_valve);
  return cmd;
}
...
void handleRoot(){
  ...
  str += F("<meta http-equiv='Refresh' content='15' />");
...
void ctrl_mode(){
  ...
  str += F("<body>");
  str += F("<p><font class='ctrl_txt'> Режим управления: </font>");
  if(ctrl == 0) str += F("<font class='ctrl_local'> ЛОКАЛЬНЫЙ");
  ...
  str += F("<p><a href='/ctrl_mode_set?ctrl=local'><button class='ctrl_local'> ЛОКАЛЬНЫЙ </button></a></p>");
...
void ventilation_adj(){
  ...
  if (ctrl == 2) {
    str += F("<a href='/ventilation_ctrl?cmd=on'><button class='ctrl_on'> ВКЛЮЧИТЬ </a>&nbsp");
...
void reconnect() {
  ...
  if(((millis() - event_reconnect) >= 60000) || first_scan){  
    ...
    //if (WiFi.status() != WL_CONNECTED) {  // если подключение к роутеру отсутствует, 
      ...
      //WiFi.mode(WIFI_AP); // тогда режим AP 
    //}
    //else {  // иначе,
      //WiFi.disconnect();
