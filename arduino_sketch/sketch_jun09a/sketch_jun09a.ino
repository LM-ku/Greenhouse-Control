...
void handleRoot(){
  ...
  str += F("<p>WiFi SSID: &nbsp <font class=\"ctrl\">");
  //str += F("<p><font class=\"ctrl\">"); 
  str += String(ssid);
  str += F("</font>");
  if(WiFi.status() == WL_CONNECTED) {
    str += F("<font color = \"#009900\"> &nbsp CONNECTED !</font></p>");
    str += F("<p>IP addres: ");
    str += F("<p><font class=\"ctrl\">"); 
    ...
    str += F("</font>");
    if(client.connect("BigGreenHouse")) str += F("<font color = \"#009900\"> &nbsp CONNECTED !</font></p>");
    else str += F("<font color = \"#FF0000\"> &nbsp NOT CONNECTED !!!</font></p>");
  }
  else str += F("<font color = \"#FF0000\"> &nbsp NOT CONNECTED !!!</font></p>");
    
  // *** На страницу настроек
  str += F("<form method=\"GET\" action=\"/comm_update\">");
...
void date_time(){
  ...
  str += F("<head>");
  str += F("<meta charset=\"UTF-8\" name=\"viewport\" content=\"width=device-width, initial-scale=1\">");
  str += F("<title>УСТАНОВКА ДАТЫ И ВРЕМЕНИ</title>");
  str += F("<style type=\"text/css\">");
...
void date_time_set() {
  ...
  str += F("<html>");
  str += F("<head>");
  str += F("<meta http-equiv=\"refresh\" content=\"1; http:/date_time\">");
  str += F("</head>");
  str += F("</html>\n\r");
  delay(100);          
  server.send ( 200, "text/html", str );
...
void ctrl_mode(){
  ...
  str += F("<head>");
  str += F("<meta charset=\"UTF-8\" name=\"viewport\" content=\"width=device-width, initial-scale=1\">");
  str += F("<title>НАСТРОЙКА РЕЖИМА УПРАВЛЕНИЯ</title>");
  str += F("<style type=\"text/css\">");
  str += F(" body {background-color: #FFFFFF; font-family: Arial, Helvetica, Sans-Serif; Color: #000000; font-size: 10pt;}");
  str += F(" .btn {font-family: Arial, Helvetica, Sans-Serif; Color: #000000; font-size: 100%;}");
  str += F("</style>");
