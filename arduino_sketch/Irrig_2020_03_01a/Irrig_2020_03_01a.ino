#define FW_VERSION "FW 27.08.2019"
...
long start_irrig; // старт полива
long end_irrig; // окончание полива
...
//-----------------------------------------------------------------------------  
//                 WEB-СЕРВЕР. ГЛАВНАЯ СТРАНИЦА. ОРОШЕНИЕ
//-----------------------------------------------------------------------------
void handleRoot(){
  String str = "";
  str += F("<!DOCTYPE HTML>");
  ...
  */
  ...
  if(first_irrig) str += F(" неизвестно </font></p>");
  else{
    after_irrig = (millis() - end_irrig)/1000; // прошло времени после окончания полива
  ...
  // *** клапан ОТКРЫТ
  if(irrig_state) {  
    irrig_cont = (millis() - start_irrig)/1000; // определение продолжительности  полива
    if(!first_irrig) {
  ...
  // *** клапан ЗАКРЫТ
  if(!irrig_state) {  
    irrig_cont = (start_irrig - start_irrig)/1000; // определение продолжительности  полива
    if(!first_irrig) {
...
void loop() {
  ...
  // *** режимы управления поливом
  if (ctrl > 2) ctrl = 2;
  if (ctrl < 0) ctrl = 0;

  if (ctrl == 0) man_valve = auto_valve;
  if (ctrl == 1) man_valve = rem_valve;
  ...
  // *** учет времени полива
  if (digitalRead(valve) && !irrig_state) {  // старт полива
    start_irrig = millis();
    irrig_state = true;
  } 
 
  if (!digitalRead(valve) && irrig_state) {  // окончание полива
    end_irrig = millis();
    irrig_state = false;
    first_irrig = false;  // сброс при включении первого же полива
  }
