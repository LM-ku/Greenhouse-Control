...
// === AccessPoint === 
#ifndef APSSID
#define APSSID "BigGreenHouse"
#define APPSK  "12345678"
#endif

const char *ap_ssid = APSSID;
const char *ap_pass = APPSK;
...
boolean irrigation = false;  // локальная команда для реле клапана полива: false = выключить, true = включить
...
void GreenHouse_control(){
  ...
  str += F("<html>\
            ...
            <body>\
            <p><font size=\"2\">Время и дата на момент обновления страницы</font></p><p>");    
  str += String(curent_time);
  str += F("</p><br /><p><b><font size=\"3\">РЕЖИМ УПРАВЛЕНИЯ</font></b></p>\
  ...
  str += F("/> Дистанционный<br /><br />\
            ...
            <p><font size=\"2\">Атмосферное давление .. ");
  str += String(pres);
  str += F(" mm.Hg </font></p>\
            <p><font size=\"2\">Температура воздуха ...... ");
  str += String(temp);
  str += F(" град.С </font></p>\
            <p><font size=\"2\">Влажность воздуха .......... ");
  str += String(hum);
  str += F(" % </font></p>");
  ...
  //}
  str += F("<p><font size=\"2\">Влажность почвы ............. ");
  str += String(moist);
  str += F(" % </font></p>");
  //if(digitalRead(14) == HIGH && mode > 0) {
  ...
  //}
  str += F("<p><font size=\"2\">после предыдущего полива прошло дней : ");
  //str += String(last_irrig_day);
  str += F(" часов : ");
  //str += String(last_irrig_hour);
  str += F(" </font></p><hr /><form action=\"/setting\" method=\"POST\"><input type=\"text\" style=\"width: 50\" name=\"set_temp\" value=\"");
  //str += float_to_String(set_temp);
  str += F("\n\"/> уставка температуры воздуха<br /><br /><input type=\"text\" style=\"width: 50\" name=\"set_hum\" value=\"");
  //str += float_to_String(set_hum);
  str += F("\n\"/> уставка влажности воздуха<br /><br /><input type=\"text\" style=\"width: 50\" name=\"set_moist\" value=\"");
  //str += float_to_String(set_moist);
  str += F("\n\"/> уставка влажности почвы<br /><br /><input type=\"submit\" value=\"Сохранить уставки\" /></form><hr /><br /><br /><form action=\"/WiFi_setup\" method=\"GET\"><input type=\"submit\" value=\"WiFi настройки\" /></form><br /><br /></body></html>\n\r");
  server.send ( 200, "text/html", str );   
}
//=============================================================================

//==== ФУНКЦИЯ ВЫБОРА НАСТРОЕК WiFi И MQTT =====================================
void handleRoot() {
  String str = "";
  str += html_header;
  /* 
  //str += F("<p>Выберите сеть к которой хотите подключиться</p>");  
  str += str += F("<b>Настройки подключения WiFi.</b></br>");
  str += F("Текущий сохраненный профиль WiFi: ");  
  if(len_ssid < 32)    str += ssid;
  else    str += F("не установлен");
    //str += F("");
  str += F("</br>Список доступных WiFi сетей: (обновляется только при первом включении!)</br>");
  str += wifi_scan;
  */ 
