#define FW_VERSION "Ventilation_2020-03-19"
...
void sensors_read(){
  ...
  if (bme.begin()) {
    ...
    // *** Команда управления реле вентилятора в автономном режиме
    if (temp >= set_temp + 1.0) auto_cmd = true; // включить вентилятора автоматически, если жарко
...
void publishData() {
  ...
  if(mqtt_send) {
    ...
    switch (step_mqtt_send) {
      ...
      case 0:    
        topic = "BigGreenHouse/ventilation/status/cmd";
        String(fan_ctrl()).toCharArray(message, 10);
        //if (!fan_ctrl()) message = "false";
        //if (fan_ctrl()) message = "true";
        //client.publish(topic.c_str(), String(fan_ctrl()).c_str());
        break;
     ...
     case 4:      
       ...
       dtostrf(pres, 8, 2, message); 
       //client.publish(topic.c_str(), message);
       break;
        
      // *** Статус датчика BME280       
      case 5:
        topic = "BigGreenHouse/ventilation/status/bme_sensor";
        String(bme_sensor).toCharArray(message, 10);
        //if (!bme_sensor) message = "error";
        //if (bme_sensor) message = "ok";
...
void connect() {
  ...
  if (WiFi.status() != WL_CONNECTED) {        // WiFi.status() == 3 : WL_CONNECTED
    ...
    else else WiFi.softAP(ap_ssid, ap_pass);  // включить точку доступа, если соединение с роутером отсутствует
  }
  ...
  if (WiFi.isConnected() && !client.connected()) {
    ...
    if (client.connect(mqtt_client.c_str())) {               
      ...
      topic = "BigGreenHouse/ventilation/ctrl/cmd";
...
void loop() {
  ...
  if (mode_ctrl == 0) man_cmd = auto_cmd;
  if (mode_ctrl == 1) man_cmd = rem_cmd;
  ...
  if (millis() - event_serial_output > 20000) { // повторять через 20 секунд
    event_serial_output = millis();
    Serial.print("mode_ctrl = ");  
    Serial.println(mode_ctrl);
