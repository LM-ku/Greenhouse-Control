#define FW_VERSION "Watering_2020.03.15"
...
void publishData() {
  if(mqtt_send) {
    ...
    case 0:    
      client.publish("/BigGreenHouse/status/moisture", moisture);
      Serial.print("MQTT client publish moisture = ");
      Serial.println(moisture);
      break;
    case 1:    
      client.publish("/BigGreenHouse/status/watering", String(valve_ctrl()).c_str());
      Serial.print("MQTT client publish valve_ctrl = ");
...
void connect() {
  ...
  if (WiFi.isConnected() && !client.connected()) {
    ...
    if (client.connect(mqtt_client.c_str())) {    
      ...
      String topic = "BigGreenHouse/cmd/watering_" + String(n);
...
void setup() {
  ...
  if (eeprom_ok) {
    ...
    EEPROM.get(136,air_val);      // уставка 100% для датчика влажности почвы
  }
  else {
    ctrl = 0;         // режим: auto
