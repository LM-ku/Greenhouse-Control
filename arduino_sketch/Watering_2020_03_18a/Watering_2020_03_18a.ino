...
boolean watering_ok = false;          // полив состоялся - для учета времени
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (str_topic == "BigGreenHouse/cmd/#" + String(n) + "_watering") {
    Serial.print("*** message = ");
    Serial.println(message);
    rem_cmd = (message == "1" || message == "true");
  }
  if (str_topic == "BigGreenHouse/cmd/#" + String(n) + "_watering_ctrl") {
    Serial.print("*** message = ");
    Serial.println(message);
  ...
  if (mqtt_send) {
    ...
    switch (step_mqtt_send) {
      ...
      case 2:
        ...
        break;
      case 3:
        topic = "BigGreenHouse/status/#" + String(n) + "_watering_ctrl";
        client.publish(topic.c_str(), String(ctrl).c_str());
        Serial.print(topic);
        Serial.print(" = ");
        Serial.println(ctrl);
        break;
...
void connect() {
  ...
  if (WiFi.isConnected() && !client.connected()) {
    ...
    if (client.connect(mqtt_client.c_str())) {
      ...
      topic = "BigGreenHouse/cmd/#1_watering_ctrl";
      client.subscribe(topic.c_str());  // подписка на топики

      Serial.print("MQTT client.subscribe = ");     // подключение к брокеру отсутствует
