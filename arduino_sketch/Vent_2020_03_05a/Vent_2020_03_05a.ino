#define FW_VERSION "FW 05.03.2020"
...
/*
  ...
  addr 141        - день окончания предыдущего полива
...
const char* ventilation_str = "<!DOCTYPE HTML><html><head><meta http-equiv='refresh' content='1; /ventilation_set'></head></html>\n\r";
...
// ===== WEB-СЕРВЕР. ГЛАВНАЯ СТРАНИЦА. СОСТОЯНИЕ ВЕНТИЛЯЦИИ =====
void handleRoot(){
  String str = "";
  str += F("<!DOCTYPE HTML>");
  str += F("<html>");
  ...
  str += F("<p><font class='ctrl_txt'> Tемпература (+/- 1.0) </font>");
  str += F("<input type='TEXT' class='form-style'  name='set_temp' value=' "); 
  str += float_to_String(set_temp);
  str += F("\n'/> °С");
  str += F("<p><font class='ctrl_txt'> Влажность не более </font>");
  str += F("<input type='TEXT' class='form-style' name='set_hum' value=' ");
  str += float_to_String(set_hum);
  str +=F("\n'/>  % </p>");
  str +=F("<div class='align_center'>");
  str += F("<p><input type='SUBMIT' class='comm' value='ПРИМЕНИТЬ УСТАВКИ' /></p>");
  str += F("</form>");
  str += F("</div>");        
...
void ventilation_set(){
  ...
  EEPROM_float_write(120, set_temp);      // сохранить в EEPROM 
  set_hum = String_to_float(server.arg(1)); 
  if (set_hum < 40.0)  set_hum = 40.0;
  if (set_hum > 90.0)  set_hum = 90.0;
  EEPROM_float_write(124, set_temp);      // сохранить в EEPROM
...
void ctrl_mode(){
  ...
  str += F("<body>");
  str += F("<p><font class='ctrl_txt'> Режим управления: </font>");
  if(ctrl == 0) str += F("<font class='ctrl_mode'> АВТОМАТИЧЕСКИЙ");
  if(ctrl == 1)  str += F("<font class='ctrl_mode'> ДИСТАНЦИОННЫЙ");
  if(ctrl == 2)  str += F("<font class='ctrl_mode'> ВРУЧНУЮ");
  str += F("</font></p><hr/>");
...
void setup() {
  ...
  if(eeprom == 170)  {  // = B#10101010
    ...
    if (len_pass > 64) len_pass = 64; // ограничение длины пароля WiFi
