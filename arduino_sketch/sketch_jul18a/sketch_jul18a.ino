#define FW_VERSION "FW 18.07.2019"
...
//-----------------------------------------------------------------------------
void fan_ctrl() {
  fan_cmd = ((ctrl==0) && local_fan) || ((ctrl==1) && rem_fan) || ((ctrl==2) && man_fan);
}


//-----------------------------------------------------------------------------
//                         УПРАВЛЕНИЕ РЕЛЕ КЛАПАНА
//-----------------------------------------------------------------------------
void valve_ctrl() {
  valve_cmd = ((ctrl==0) && local_valve) || ((ctrl==1) && rem_valve) || ((ctrl==2) && man_valve);
}
...
void handleRoot(){
  ...
  if(ctrl == 0) str += F("ctrl_local'>ЛОКАЛЬНЫЙ");
  ...
  if(fan_cmd)  str += F("ctrl_on' value='ВКЛЮЧЕНА");
  if(!fan_cmd) str += F("ctrl_off' value='ВЫКЛЮЧЕНА");
  str += F("'/></p>");
  str += F("</form><hr />");
...
String str = "";
  ...
  //str += F("<p><a href='/' class='btn'> НА ГЛАВНЫЙ ЭКРАН </a></p>");
...
// *** ОТКРЫТЬ клапан орошения ВРУЧНУЮ  
void irrigation_on() { 
  man_valve = true; 
...
// *** ЗАКРЫТЬ клапан орошения ВРУЧНУЮ  
void irrigation_off() { 
  man_valve = false; 
  server.send ( 200, "text/html", root_str );
  //delay(1000);   
...
// *** Изменить уставки включения полива
void irrigation_set(){
...
void loop() {
  ...
  //**************************************
  digitalWrite(ESP_BUILTIN_LED, !fan_cmd);
  //**************************************
