#define FW_VERSION "FW 17.07.2019"
...
byte ctrl; // режим: 0 = local, 1 = remote, 2 = manual 
boolean man_ctrl;
boolean man_fan;
boolean man_valve;
boolean irrig_state;
...
boolean local_fan;  // локальная команда для реле вентилятора: false = выключить, true = включить
...
void handleRoot(){
  ...
  str += F(" width: 24px;");
  str += F(" height:30px;");
  str += F("  margin-left: -2.0em;");
  str += F("  vertical-align: middle;");
  str += F("  background: url(data:image/png;base64,) no-repeat;");
  str += F("}");
  ...
  if(ctrl == 0) str += F("ctrl_off' value='ЛОКАЛЬНЫЙ' /></p>");
  if(ctrl == 1) str += F("ctrl_on' value='ДИСТАНЦИОННЫЙ' /></p>");
  if(ctrl == 2) str += F("ctrl_off' value='ВРУЧНУЮ' /></p>");
  str += F("</form><hr />");
  ...
  if(digitalRead(valve)) {  
    ...
    str += F(" мин.</font></p>");
    str += F("<p><font class='ctrl_txt'> Клапан полива: ");
...
//-----------------------------------------------------------------------------  
//           WEB-СЕРВЕР. ОГРАНИЧЕНИЕ ДОСТУПА К НАСТРОЙКАМ И УПРАВЛЕНИЮ
//-----------------------------------------------------------------------------
...
String str = "";
  ...
  str += F("<h1>ВВЕДИТЕ ПАРОЛЬ</h1>");
  str += F("<form action='pass_read' method='POST'>");
  
  str += F("<input type='TEXT' name='psk' placeholder='Password' />");
  
  str += F("<input type='SUBMIT' value='ДАЛЕЕ' />");
  str += F("</form>");
...
void ctrl_mode(){
  ...
  str += F("<body>");
  str += F("<p><font class='ctrl_txt'> Режим управления: </font>");
  if(ctrl == 0) str += F("<font class='ctrl_off'> ЛОКАЛЬНЫЙ");
...
void WiFi_setup() {
  ...
  String wifi_scan = "";
  if (n == 0) {
    wifi_scan += F("<h2>СЕТИ  WiFi НЕ НАЙДЕНЫ</h2>");
  }
  else {
    wifi_scan += F("<h2>НАЙДЕНЫ СЕТИ  WiFi</h2>");
    str += wifi_scan;  
    for (int i = 0; i < n; ++i) {
      String ssid_scan = WiFi.SSID(i);
      if (ssid_scan.length() == 0)  ssid_scan = "< ??? >";
...
void http_server() {  
    ...
    server.on("/pass_ctrl", pass_ctrl);
    // server.on("/pass_read", pass_read);
...
void setup() {
  ...
  local_fan = false;  // локальная команда для реле вентилятора = выключить
  local_valve = false;  // локальная команда для реле клапана полива = выключить  
  irrig_state = false;
  man_ctrl = false;
  man_fan = false;
  man_valve = false;
...
void loop() {
  ...
  // *** инициализация команд ручного управления
  if (ctrl == 0) {
    man_fan = local_fan;
    man_valve = local_valve;
  }
  if (ctrl == 1) {
    man_fan = rem_fan;
    man_valve = rem_valve;
  ...
  boolean fan_cmd = ((ctrl==0) && local_fan) || ((ctrl==1) && rem_fan) || ((ctrl==2) && man_fan);
  digitalWrite(fan, fan_cmd);
 
  // *** включение реле клапана полива 
  boolean valve_cmd = ((ctrl==0) && local_valve) || ((ctrl==1) && rem_valve) || ((ctrl==2) && man_valve);
