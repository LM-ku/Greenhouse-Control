#define FW_VERSION "FW 08.03.2020"
...
void upd() {
  ...
  str += F(".btn-tertiary{color:#555;padding:0;line-height:40px;width:300px;margin:auto;display:block;border:2px solid #555}");
  str += F(".btn-tertiary:hover,.example-2 .btn-tertiary:focus{color:#888;border-color:#888}");
  str += F(".input-file{width:.1px;height:.1px;opacity:0;overflow:hidden;position:absolute;z-index:-1}");
  str += F(".input-file + .js-labelFile{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding:0 10px;cursor:pointer}");
  str += F(".input-file + .js-labelFile .icon:before{content:\"\f093\"}");
  str += F(".input-file + .js-labelFile.has-file .icon:before{content:\"\f00c\";color:#5AAC7B}");
  ...
  str += F("function() {");
  str += F("'use strict';");
  str += F("$('.input-file').each(function() {");
  str += F("var $input = $(this),");
  str += F("$label = $input.next('.js-labelFile'),");
  str += F("labelVal = $label.html();");
  str += F("$input.on('change', function(element) {");
  str += F("var fileName = '';");
  str += F("if (element.target.value) fileName = element.target.value.split('\\').pop();");
  str += F("fileName ? $label.addClass('has-file').find('.js-fileName').html(fileName) : $label.removeClass('has-file').html(labelVal);");
  str += F("});");
  str += F("});");
  str += F("}");
  str += F("</script>");
...
//upd_str = "<form method='POST' action='/update' enctype='multipart/form-data'><input type='file' name='update' /><input type='submit' value='Start update' /></form>";  

  str += F("<h2>ОБНОВЛЕНИЕ ПО</h2>");

 /* 
  str += F("<form  method='POST' action='/update' enctype='multipart/form-data'>");
  //str += F("<div class='example-2'>");
  //str += F("<div class='form-group'>");
  str += F("<input type='FILE' name='update' id='file' class='input-file'>");
  str += F("<label for='file' class='btn btn-tertiary js-labelFile'>");
  str += F("<i class='icon fa fa-check'></i>");
  str += F("<span class='js-fileName'>Загрузить файл</span>");
  str += F("</label>");
  //str += F("</div>");
  //str += F("</div>");
*/ 
...
void handleRoot(){
  ...
  str += F("<title>ВЕНТИЛЯЦИЯ ТЕПЛИЦЫ</title>");
  str += F("<style type='text/css'>");
  
  str += F("body {"); 
  str += F(" display: inline-block;");
  //str += F("  max-width: 325px;");
  str += F(" background-color: #FFFFFF;"); 
  str += F(" font-family: Arial, Helvetica, Sans-Serif;"); 
  str += F(" margin-left: 20px;"); 
  str += F(" Color: #000000;"); 
...
void reconnect() {
  ...
  if(((millis() - event_reconnect) >= 60000) || first_scan){  
    ...
    if (WiFi.status() == WL_CONNECTED) {  // если есть подключение к роутеру 
      if (ssid == "WiHome" || ssid == "Keenetic-2927") WiFi.mode(WIFI_STA); 
