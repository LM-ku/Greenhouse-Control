#define FW_VERSION "FW 28.05.2019"
...
//-----------------------------------------------------------------------------
//                ФОРМИРОВАНИЕ СИМВОЛЬНОГО ВЫВОДА МИНУТ И СЕКУНД
//-----------------------------------------------------------------------------
...
void handleRoot(){
  ...
  str += F("<?php header(\"refresh: 15\"); ?>\
            ...
            <body>\
            <hr />\
            <p><font size=\"2\"><b>Дата : &nbsp&nbsp");
  str += String(day())+"."+String(month())+"."+String(year())+"г. &nbsp&nbsp Время : &nbsp&nbsp"+String(hour())+digits_to_String(minute())+digits_to_String(second());
  str += F("</b></font></p><hr />\
            <p>Режим управления : ");
...
//----------------------------------------------------------------------------- 
void reconnect() {
  if((millis() - event_mqtt_connect) > 600000){  // повторять через 10 минут
...
//-----------------------------------------------------------------------------
//                          РАБОТА С NTP-СЕРВЕРОМ 
//-----------------------------------------------------------------------------
 
time_t getNtpTime() {
  detachInterrupt(digitalPinToInterrupt(tmr_opn));  // деактивировать прерывание на время NTP-запроса 
  detachInterrupt(digitalPinToInterrupt(tmr_cls));  // деактивировать прерывание на время NTP-запроса    
